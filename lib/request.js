'use strict'

const qs = require('querystring')
const { Cookie } = require('@vikhola/cookies')
const { parse, serialize } = require('fast-uri')
const { HttpHeaders } = require('./headers.js')

const kRequestRaw = Symbol('kRequestRaw')
const kFeatures = Symbol('kFeatures')
const kRequestOriginalUrl = Symbol('kRequestOriginalUrl')

const kRequestCookieHeader = 'Cookie'

class HttpRequest {

    constructor(features, request, params, body) {
        this[kFeatures] = features
        this[kRequestRaw] = request

        this._url = this.raw.url
        this._body = body
        this._query = {}
        this._params = params
        this._headers = new HttpHeaders(this.raw.headers)
        this._cookies = Cookie.from(this.headers.get(kRequestCookieHeader))
    }

    get raw() {
        return this[kRequestRaw]
    }

    get body() {
        return this._body.content
    }

    get headers() {
        return this._headers
    }

    get cookies() {
        return this._cookies
    }

    get method() {
        return this.raw.method
    }

    get socket() {
        return this.raw.socket
    }

    get ip() {
        return this.socket.remoteAddress || ''
    }

    get contentType() {
        return this.headers.get('Content-Type')
    }

    get contentLength() {
        const header = this.headers.get('Content-Length')
        return header ? parseInt(header) : 0
    }

    get protocol() {
        return this.socket.encrypted ? 'https' : 'http'
    }

    get secure () {
        return this.protocol === 'https'
    }

    get host() {
        return this.raw.httpVersionMajor >= 2 ? this.headers.get(':authority', 'host') : this.headers.get('host')
    }

    get originalUrl() {
        if (!this[kRequestOriginalUrl]) 
            this[kRequestOriginalUrl] = this.raw.originalUrl || this.raw.url
        
        return this[kRequestOriginalUrl]
    }

    get url() {
        return this._url
    }

    set url(url = '') {
        this._url = url
    }

    get path() {
        return parse(this.url).path
    }

    set path(path) {
        const url = parse(this.url)
        if(url.path === path)
            return

        url.path = path
        this.url = serialize(url)
    }

    get params() {
        return this._params
    }

    get querystring() {
        return parse(this.url).query
    }
    
    set querystring(querystring) {
        const url = parse(this.url)

        if (url.query === querystring) 
            return

        url.query = querystring
        this.url = serialize(url)
    }

    get query() {
        const str = this.querystring
        const c = this._query
        return c[str] || (c[str] = qs.parse(str))
    }

    set query(query) {
        this.querystring = qs.stringify(query)
    }

}

module.exports = { HttpRequest }